(() => {
  ...
  const pages = [
    { href: 'index.html', label: 'Убрать водяной знак' },
    { href: 'merge-pdf.html', label: 'Объединить PDF' },
    { href: 'compress-webp.html', label: 'Сжать в&nbsp;WebP' },
  ];

  const pathname = (window.location && window.location.pathname) ? window.location.pathname : '';
  const currentHref = pages.find((p) => pathname.endsWith(p.href))?.href || '';
  let normalizedCurrentHref = currentHref;
  if (!normalizedCurrentHref && (pathname === '/' || pathname === '')) {
  ...
  const internalNavHtml = pages
    .map((p) => {
      if (p.href === normalizedCurrentHref) {
        // Текущую страницу делаем не-кликабельной, но визуально показывает “где ты”.
        return `<span class="footer-pages-current" aria-current="page">${p.label}</span>`;
      }
      return `<a href="${p.href}">${p.label}</a>`;
    })
    .join('<span class="footer-pages-sep" aria-hidden="true">•</span>');

  footer.innerHTML = `
    <div class="footer-note footer-pages">
      ${internalNavHtml}
    </div>
